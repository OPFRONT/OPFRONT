...
(function() {
    // Only apply settimeout workaround for iOS - for all others, we map to native Timers
    if(!navigator.platform.match(/iP(hone|od|ad)/)) return;
        ...
        _touchTimer = function(callback, wait, isInterval) {
            ...
            function _loop() {
                ...
                if(timer[uid].loop !== false) {
                    timer[uid].requestededFrame = webkitRequestAnimationFrame(_loop, null);
                    ...
                    if(interval) {
                        delta = new Date().getTime() + wait;
                        timer[uid].loop = now <= delta;
                        timer[uid].requestedFrame = webkitRequestAnimationFrame(_loop, null);
                    } else {
                        delete timer[uid];
                    }
                }
